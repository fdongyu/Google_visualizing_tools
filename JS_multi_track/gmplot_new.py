# -*- coding: utf-8 -*-
"""
Created on Wed Jul 25 11:05:20 2018
...
class GoogleMapPlotter(object):
    ...
    def plot(self, lats, lngs, color=None, c=None, name=None, **kwargs):
        color = color or c
        kwargs.setdefault("color", color)
    ...
    def draw(self, htmlfile, legend=True):
        ...
        if legend==True:
            ...
            f.write('#legend {font-family: Arial, sans-serif;background: #fff;padding: 8px;border: 3px solid #999;position: absolute;left: 1%;top: 50px;z-index: 5;}\n')
            f.write('</style>\n')

        f.write('<script type="text/javascript">\n')
        f.write('\tfunction initialize() {\n')
        f.write('\t\tvar icons = [];\n')
        self.write_map(f, legend)
        self.write_grids(f)
        self.write_points(f)
        self.write_paths(f, legend)
        ...
        if legend==True:
            ...
            f.write('for (var type in icons) {\n')
            f.write('var name = icons[type].name;\n')
            f.write('var iconcolor = icons[type].color;\n')
            f.write("var div = document.createElement('div');\n")
            f.write("div.innerHTML = '<span style="+ '"color:' + "'+iconcolor+';font-size: 1.2em;"+'">&#9644</span>'+"'+name;\n")
    ...
    def write_paths(self, f, legend):
        for path, settings in self.paths:
            self.write_polyline(f, path, settings, legend)
    ...
    def write_polyline(self, f, path, settings, legend):
        clickable = False
        geodesic = True
        strokeColor = settings.get('color') or settings.get('edge_color')
        strokeOpacity = settings.get('edge_alpha')
    ...
    def write_legend(self,f, settings):
        """
        write legend
        """        
        strokeColor = settings.get('color')
        strokeName = settings.get('name')
        f.write('\n')
        f.write("var icon = {name:'"+strokeName+"',color:'"+strokeColor+"'};\n")
        f.write('icons.push(icon);\n')
        f.write('\n')
